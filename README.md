...
1. インストーラーをダウンロードし、MODマネージャーでインストール。
2. 起動したウィザードに従い、好みのプリセットやオプションを選択。
3. インストール完了後、そのままゲームを起動して反映を確認してください。
...
別途、好みのフォントSWFファイルとフォント設定テンプレートをダウンロードし、下記フォルダへ配置してください。
`Skyrim\Data\Interface`
...
|**ベースUI**|`_every` (汎用) / `_book` (書籍) / `_handwrite` (手紙)|元のゲームフォントに合わせてサイズが調整されています|
